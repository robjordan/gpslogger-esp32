...
#define GPS_BLINK_RED GPIO_NUM_32
#define GPS_BLINK_GREEN GPIO_NUM_33
...
void gps_blink(unsigned led, unsigned ontime, unsigned offtime, unsigned repeats) {
    while (repeats > 0) {
...
static void gps_handle_nmea_message(uint8_t *buf, size_t len) {
    ...
    ESP_LOG_BUFFER_HEXDUMP(TAG, buf, len, ESP_LOG_INFO);
    const char *resetMsg = "$GNTXT,01,01,02,u-blox AG - www.u-blox.com";

    /* we are not supposed to get NMEA messages, it looks like the device has reset, re-initialise */
    if (memcmp(buf, resetMsg, strlen(resetMsg)) == 0) {
	ESP_LOGI(TAG, "re-sending UBX init string");
	gps_init();
    }
}
...
static void gps_handle_message(uint8_t *buf, size_t len) {
    ...
    /* consider the possibility that this could be either a UBX or NMEA message */
	...
	if ((*p == 0xb5) && (*(p+1) == 0x62)) {
	...
	} else if (*p == '$' || *p == '!') { /* these are the two valid NMEA start delimiters */
	    /* find the end delimiter, which is CR LF */
	    for (msglen = 0; (msglen < (len-(p-buf))); msglen++) {
		if (*(p+msglen) == 0x0d && *(p+msglen+1) == 0x0a) {
		    /* we found the terminal delimiter */
		    msglen += 2; 	/* because it's a two character delimiter */
		    gps_handle_nmea_message(p, msglen);
		    break;
		}
	    }
	    ...
	    /* either we got garbage, or we have not found the start of the message */
...
void gps_main()
{
    ...
    // Setup a GPIO to blink LED when a valid/invaid GPS reading is received
    gpio_pad_select_gpio(GPS_BLINK_GREEN);
    ESP_ERROR_CHECK(gpio_set_direction(GPS_BLINK_GREEN, GPIO_MODE_OUTPUT));
    gpio_pad_select_gpio(GPS_BLINK_RED);
    ESP_ERROR_CHECK(gpio_set_direction(GPS_BLINK_RED, GPIO_MODE_OUTPUT));
